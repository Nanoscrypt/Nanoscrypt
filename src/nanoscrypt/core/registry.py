...
from nanoscrypt.models.database import Base, DBTool, DBToolVersion, DBToolExecution
        ...
        async with self.session_factory() as session:
            async with session.begin():
                ...
                if not db_tool:
                    ...
                    db_tool = DBTool(
                        name=tool.name,
                        purpose=tool.manifest.input_schema.get("purpose") or tool.readme[:200],
                    ...
                    db_tool.dependencies = list(set(db_tool.dependencies + tool.manifest.dependencies))
    ...
    async def update_stats(
        ...
        async with self.session_factory() as session:
            async with session.begin():
                ...
                stmt_all = select(DBToolExecution.success).where(DBToolExecution.tool_id == db_tool.id)
            ...
            await session.commit()
            log.info("registry_stats_updated", total_runs=total_runs, success_rate=db_tool.success_rate)
