...
from nanoscrypt.llm.prompts.repair import TOOL_REPAIR_SYSTEM_PROMPT, TOOL_REPAIR_USER_TEMPLATE
...
class RepairLoop:
    ...
    def run_tests_in_sandbox(self, session_id: str, tool: GeneratedTool) -> ExecutionResult:
    ...
    async def repair_tool(
        ...
        """Tries to repair the tool code up to max_attempts times."""
        ...
        while attempt < self.max_attempts:
            ...
            try:
                ...
                current_failure = ExecutionResult(
                    stdout="",
                    stderr=f"Repair loop logic error: {str(e)}",
                    return_code=-1,
                    runtime_ms=0,
                    timed_out=False,
                    workspace_path=failure_result.workspace_path
