...
- `direct_response`: Choose this ONLY if the user is asking a purely conversational or conceptual question (e.g. 'what is Nanoscrypt?'). NEVER select direct_response if the user requests ANY action such as creating, deleting, writing, or managing files/folders in the workspace. For file/folder creation or modification, you MUST select `generate_tool` (or `reuse_tool`).
